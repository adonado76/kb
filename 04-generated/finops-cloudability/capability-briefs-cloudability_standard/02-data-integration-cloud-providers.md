...
**Google Cloud — soporta GCP Standard Billing y GCP Detailed Billing**, con credencialización por lotes igual que AWS, un agente de monitoreo dedicado para habilitar Rightsizing, manejo de tags/labels, soporte de precios personalizados (custom pricing) y configuración específica del Commitment Portfolio para GCP. Para una cuenta **GCP Payer**, el export recomendado al credencializar es **GCP Detailed Billing**: es el que entrega visibilidad completa de las mejoras **SCUD** en la interfaz de reporte de Cloudability, incluyendo los escenarios de sobre-utilización y sub-utilización. Trampa operativa documentada con precisión: si el cliente tiene **VPC Service Controls** habilitado, las capacidades avanzadas (Rightsizing, Commitments) dejan de funcionar **aunque el chequeo de estado de la credencial aparezca en verde** — ese chequeo solo valida la asignación del rol IAM, no valida el acceso al perímetro de APIs, y VPC Service Controls crea un perímetro de seguridad que bloquea a la cuenta de servicio de Cloudability para alcanzar las APIs de Google que las capacidades avanzadas requieren, independientemente de la configuración IAM. La solución documentada es agregar una **regla de ingreso (ingress rule)** al perímetro de VPC Service Controls para la identidad de la cuenta de servicio de Cloudability — `billing-data-service-acct@cloudability-credentials.iam.gserviceaccount.com` — y asegurar que el perímetro permita las APIs que Cloudability usa; los ejemplos comunes documentados son `compute.googleapis.com`, `recommender.googleapis.com`, `cloudbilling.googleapis.com` y `monitoring.googleapis.com`.

**Oracle Cloud (OCI)** — credencialización vía federación de usuario, un mecanismo distinto a los tres proveedores anteriores.

**IBM Cloud y un método de credencialización genérico** — un método propio de Cloudability para credencializar cuentas, más una alternativa vía **Deployable Architecture (DA) App** — sugiriendo que IBM está estandarizando el modelo de credencialización hacia un mecanismo más moderno de infraestructura-como-código, en paralelo al método clásico. Detalle de captura del método Cloudability (Settings → Vendor Credentials → Add Datasource → IBM → Next → *Generate Template*): el **Cost Report Name no debe llevar la extensión del archivo (`.json`)**, y si lo que se está conectando es una **IBM Cloud Standard Account**, el campo **Enterprise Id se deja en blanco**.
...
En GCP, la pregunta "¿tienen VPC Service Controls habilitado?" pertenece al discovery técnico inicial y no al troubleshooting posterior — es el caso documentado donde la credencial se ve **verde y correcta** pero Rightsizing y Commitments simplemente no producen resultados, porque el chequeo de estado no valida el perímetro de APIs. Saberlo de antemano evita un ciclo completo de diagnóstico contra un síntoma que no se parece a un problema de credenciales.
