...
**Objective:** Programmatically retrieve data from TBM Studio tables and reports using the
Platform API

**When to use:** When you need flexible, on-demand access to TBM Studio data for custom
integrations, ETL pipelines, or automation scripts

**Prerequisites:**

- TBM Studio version 12.0 or later
- API authentication credentials (API keys or user credentials)
- Appropriate permissions for downloading data (AccessProd, AccessStg, DrillDown, and
  relevant View permissions)
...
The TBM Studio Platform API provides direct, programmatic access to your data without
requiring the DataLink product. Common reasons to use the API include:
...
Before making API calls, configure your authentication. TBM Studio supports API key
authentication (recommended) and username/password authentication.

**For API key authentication:**

1. In Enhanced Access Administration, ensure your user account has API keys
   configured.
2. Note your **public key** and **secret key** values.
3. Ensure the key has been granted access to the appropriate environment.
...
Note: Not all View permissions may be required depending on the data you're downloading. Test
...
**Response:** The response includes an apptio-opentoken cookie that you'll use for
subsequent requests.
...
Replace [yourdomain.com] with your domain and [yourfrontdoorenv] with your environment name
(typically "main").
...
**Note:** The app-type and app-version headers are required for TBM Studio version 12.9
and later.
...
In version 12.9+, forgetting app-type and app-version headers causes authentication
failures.

**Incorrect time period format**

Time periods must follow Apptio conventions (e.g., "January:2024" or "current" for the
current month).
...
Download failures often indicate missing permissions. Verify your account has the required
permission set listed in Step 1.

## What's Next

- For comprehensive API documentation including upload capabilities, see Section 5.5 (API
  Reference)
- For DataLink integration patterns, see the DataLink [documentation](https://www.ibm.com/docs/en/apptio-platform/datalink/saas?topic=datalink "(Opens in a new tab or window)")
- To set up Published Tables for automated BI feeds, see [here](htg-config-pt.html).
