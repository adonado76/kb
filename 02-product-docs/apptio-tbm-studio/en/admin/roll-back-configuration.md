...
breadcrumb: []
source_path: "admin/roll-back-configuration.html"
images: []
capability_ids: []
...
- Something was deployed to production prematurely. See [Check out and check in best practices](bp-check-out.html "(Opens in a new tab or window)") for information on how
...
practices](bp-check-out.html "(Opens in a new tab or window)").

1. Select the document in Project Explorer.
2. On the Build tab select Check In History.
3. Right-click on the check in you want to roll back to and select Rollback
   To.
...
4. Enter your reason for rolling back (it is recommended to be as descriptive as possible), and
   ...
   After the rollback, the
   result is displayed similar to the example below:

   ![](../../resources/images/studio_images/rollback-result.png)

   You can see the calculation
   status in the Builds tab:

   ![](../../resources/images/studio_images/changehist-rollback.png)

   Note: All check ins after
   the check in selected for rollback are undone with the exception of the branch closure (branch
   operations are exempt from a rollback).
...
To do this, you would use a branch. See [Check out and check in best practices](bp-check-out.html "(Opens in a new tab or window)") to understand how to create a branch.
