...
| [TimePeriod](functions/timeperiod.html "Devuelve el valor de una métrica especificada al comienzo de un período de tiempo (por ejemplo, mes, trimestre, año) que es un número especificado de períodos antes o después del período actual. La unidad de tiempo viene determinada por la vista seleccionada o por un sufijo de granularidad explícito.") | `TimePeriod(metric,time)` |
