...
data templates and table data (see [Manage dimensions](manage-reference-data.html)).

![image](../../resources/images/it%20planning_images/icon_video.png)

Watch these videos from Apptio Education Services:

- [Configuring Reference Data: Financial Dimensions and Cost
  Object Permissions](https://community.ibm.com/community/user/viewdocument/configuring-reference-data-financi?CommunityKey=4100dfb8-fc23-4203-83c7-019253cf7c0b&tab=librarydocuments "(Opens in a new tab or window)").
- [Understanding Data Hierarchies in Planning](https://community.ibm.com/community/user/viewdocument/understanding-data-hierarchies-in-i?CommunityKey=2e85ed45-9b8a-486c-bd55-019253d466eb&tab=librarydocuments "(Opens in a new tab or window)").
...
In the Costing & Planning Plan navigation menu, select
**Configuration > Reference Data > Account**.
...
- **Cost Pool** - The [ATUM category](../../atum/home.html) to
...
- You can also substantially customize reference data dimensions and line-item tables (see [Manage custom reference data
  (dimensions, lists, and line-item tables).](manage-custom-reference.html "The tasks below can only be performed by users assigned to the Admin or Budget Process Owner roles. For additional information on roles, see Frontdoor permissions and roles.")
...
variance](analyze-budget-variance.html "The tasks below can only be performed by users assigned to the Admin or Budget Process Owner roles. For additional information on roles, see Frontdoor permissions and roles."). 10-15 discrete values are recommended for this dimension. You may already have
...
However, you can build the reference data in Costing Standard (see [Integrate with Cost Transparency](integrate-ct.html "If your organization runs both Apptio Costing Standard and an Apptio Planning application, you can integrate them to share data.")).

1. In the Costing & Planning  Plan navigation menu, select
   Configuration > Reference Data > Account Category.
...
1. In the Costing & Planning  Plan navigation menu, select
   Configuration > Reference Data > Cost Center.
2. Update the data table values as required:
...
1. In the Costing & Planning  Plan navigation menu, select
   Configuration > Reference Data > Department.
2. Update the data table values as required:
...
**SEE ALSO**: [Update department hierarchy](update-department-hierarchy.html)
...
other dimensions, such as value-added tax (VAT). See [Manage Contract reference data](manage-contract-configuration.html "The tasks below can only be performed by users assigned to the Admin or Budget Process Owner roles. For additional information on roles, see Frontdoor permissions and roles."))

1. In the Costing & Planning  Plan navigation menu, select
   **Configuration > Reference Data > Location**.
...
structured approach enables the effective roll-up reporting of variance drivers. See [Analyze budget variance](analyze-budget-variance.html "The tasks below can only be performed by users assigned to the Admin or Budget Process Owner roles. For additional information on roles, see Frontdoor permissions and roles.").
...
mappings into your Apptio Planning application. See [Integrate with Cost Transparency](integrate-ct.html "If your organization runs both Apptio Costing Standard and an Apptio Planning application, you can integrate them to share data."). Variance Driver does not currently support
importing from Costing Standard.

1. In the Costing & Planning  Plan navigation menu, select
   Configuration > Reference Data > Variance Driver.
2. Update the data table values as required:
...
1. In the Costing & Planning  Plan navigation menu, select
   **Configuration > Reference Data > Vendor**.
