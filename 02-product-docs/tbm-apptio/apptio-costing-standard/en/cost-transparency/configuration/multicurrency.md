...
multi-currency](../../studio/formulas-and-functions/functions/tablematch.html "Returns a value based on a rules table that functions like a set of IF statements. Each row in the rules table defines one rule. Conditions in the same row are combined with AND, and values within the same cell are treated as OR conditions.").
