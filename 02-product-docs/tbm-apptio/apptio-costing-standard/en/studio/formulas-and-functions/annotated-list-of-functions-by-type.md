...
| [TimePeriod](functions/timeperiod.html "Returns the value of a specified metric in the same table for a time unit (e.g., month, quarter, year) that is a specified number of units before or after the current period. An offset of 0 returns the value for the current period. The time unit is determined by the selected view or an explicit granularity suffix.") | `TimePeriod(metric,time)` |
