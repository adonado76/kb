...
3. Configure suas tabelas de entrada para tabelas mestre usando as fórmulas listadas abaixo. Se a etapa Fórmulas não for exibida, adicione-a usando a opção Adicionar etapa.
   ...
   :   - Atualize a seguinte coluna para **Active Directory** :
       ...
       - Crie e atualize as seguintes colunas para **Dispositivos de usuário final** :
         ...
         | Nome da coluna | Tipo | Formato | Fórmula |
         | --- | --- | --- | --- |
         | Unidade de negócios | Rótulo |  | =Pesquisar(ID do funcionário,Dados mestres de trabalho da EUI,ID do funcionário,Unidade de negócios) |
         | Proprietário da unidade de negócios | Rótulo |  | =Pesquisar(ID do funcionário,Dados mestres de trabalho da EUI,ID do funcionário,Proprietário da unidade de negócios) |
         | Verificação CC | Rótulo |  | =Pesquisar(ID do funcionário,Dados mestres de trabalho EUI,ID do funcionário,Centro de custos) |
         ...
         | Departamento | Rótulo |  | =Pesquisar(ID do funcionário,Dados mestres de trabalho EUI,ID do funcionário,Descrição do centro de custos) |
         | Contagem de Dispositivo | Numérico |  | =1 |
         | Inventário em estoque | Rótulo |  | =Pesquisar(Status do estoque,Definição do status do dispositivo do usuário final em estoque,Lista de status em estoque,Lista de status em estoque) |
         | S.O. | Rótulo |  | =Lookup(ID do ativo,EUI Active Directory Dados mestre,ID do ativo,SO) |
         | Versão de S.O. | Rótulo |  | =Pesquisar(ID do ativo,EUI Active Directory Dados mestre,ID do ativo,Versão do SO) |
         | Tabela de fontes | Rótulo |  | ="End User Devices Raw" (Dispositivos do usuário final brutos) |
         | Nome de usuário HR | Rótulo |  | =Pesquisar(ID do funcionário,Dados mestres de trabalho da EUI,ID do funcionário,Nome do funcionário) |
         ...
         ![imagem](../../../../../03-media/apptio-costing-standard/resources/images/studio_images/eud-1.png)
