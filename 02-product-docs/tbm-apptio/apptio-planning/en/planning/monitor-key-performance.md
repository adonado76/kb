...
breadcrumb: []
source_path: "planning/monitor-key-performance.html"
images: []
capability_ids: []
...
Note: The Dashboard must be made visible in the interface. See [Edit the Company Profile](edit-company-profile.dita "(Opens in a new tab or window)"). Once that is done, the Dashboard is
...
financial targets](set-financial-targets.dita "(Opens in a new tab or window)"). The KPIs include the following:
...
can be found on the Expenses page, Activities tab. See [Edit the Company Profile](edit-company-profile.dita "(Opens in a new tab or window)").
