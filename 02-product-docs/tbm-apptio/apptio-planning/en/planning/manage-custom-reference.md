...
language: "en"
doc_type: "planning"
title: "Custom reference data (dimensions)"
breadcrumb: []
source_path: "planning/manage-custom-reference.html"
images: []
capability_ids: []
...
- Create up to 13 custom dimensions to extend built-in dimensions (see [Planning applications: Manage
  reference data](manage-reference-data.dita "(Opens in a new tab or window)")). Use custom dimensions to define the data category sets that extend and
...
| You can rename columns in Reference Data.  [Learn more about renaming a column](manage_schema.html).  • When the column is renamed, users see the new name in line-item table column headers. | You cannot rename the column in custom lists. |
...
dimensions](manage_schema.html).

## Download and populate custom dimension table templates

You can import reference data from Costing Standard. When you import reference data, Planning
imports the most recent reference dataset regardless of the active date setting in Costing Standard.
See [Integrate with Cost
...
3. Select a table, and then select ![](../../resources/images/icons/3-dots.png)>Export.
...
1. To create new custom dimension, see [Add and manage custom attributes of dimensions](manage_schema.dita "(Opens in a new tab or window)").
...
To delete a custom dimension, see [Delete a reference data table column](manage_schema.dita "(Opens in a new tab or window)")
