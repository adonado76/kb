...
source_path: "SSIU957/it-benchmarking/troubleshooting/config-integration-issues.html"
images: []
capability_ids: []
...
1. Confirm when the last data refresh was run.
1. Verify that the Costing model you changed is actually feeding the Benchmarking data set.
...
1. Check for unmapped or partially mapped cost in the new version.
1. Treat the version-change year as a new baseline, not a continuation.
1. In Interactive Benchmarking, check that its Settings reflect the same TBM version as your Costing project.

Short answer you can send We moved to a newer taxonomy, which changes how towers and sub-towers are grouped. That makes new results not directly comparable to prior years. We’ll treat the change year as a baseline reset and, where needed, restate prior periods in the new structure.
