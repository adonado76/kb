...
breadcrumb:
  ...
  - "Save Behavior"
source_path: "SSWRJM/studio/new-uc/reference/report-studio-reference/save-behavior.html"
images: []
capability_ids: []
...
Editable tables use a two-stage save process: local save (persisting changes to the editable table) and publish (updating the associated transform table).
...
1. Click Save at the bottom of the table to persist changes
1. Saved changes update the editable table but not yet the transform pipeline
1. The Show Changes option displays all pending modifications
...
1. Confirm the publish action in the warning dialog
1. Enter a description and click Check In
1. The associated transform table updates with the new data

Publishing always overwrites the entire editable table dataset in the associated transform table.

Prerequisites: Manual publishing requires Dev access and Stage access but does not require TBM Studio or Admin access.
