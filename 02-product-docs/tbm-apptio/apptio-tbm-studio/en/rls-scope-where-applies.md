...
source_path: "SSWRJM/studio/new-uc/concepts-architecture/security-model/rls-scope.html"
images: []
capability_ids: []
...
RLS can be applied across multiple areas of TBM Studio. Understanding where it applies—and where it does not—is essential for complete security coverage.
...
Tables do not automatically inherit RLS settings. You must add an RLS step for each model step that requires data security. If you configure RLS on Cost Source but not on IT Resource Towers, users may see unfiltered data when viewing reports based on IT Resource Towers.
