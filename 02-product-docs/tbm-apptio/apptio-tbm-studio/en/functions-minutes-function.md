...
breadcrumb:
  ...
  - "Minutes function"
source_path: "SSWRJM/studio/formulas-and-functions/functions/minutes.html"
images:
  - "03-media/apptio-tbm-studio/aug120.png"
capability_ids: []
...
*Converts a date value into the number of minutes since January 1, 1970, as a numeric value.*
...
- date_format : The format of the date string. Required only if the date_expression is not in a standard supported format. OptionalAn expression that evaluates to a date to be converted to a double value. The format is MM/DD/YYYY HH:MM or any other standard date format supported by the application.

## Behavior

- Takes a date input and converts it to a numeric value representing minutes since the epoch (January 1, 1970).
- If no date_format is provided, the application assumes the input is in a supported standard format.
- The result is useful for calculating time differences, durations, or performing time-based arithmetic.
...
- If the input date is in a non-standard format, use the date_format parameter to specify the correct format.
