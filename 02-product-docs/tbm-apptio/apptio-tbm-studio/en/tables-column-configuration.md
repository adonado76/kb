...
source_path: "SSWRJM/studio/new-uc/reference/report-studio-reference/column-config.html"
images: []
capability_ids: []
...
1. Select the Format, Type, and optionally enable Summable.
1. Click OK.
...
- Position the mouse pointer over the right boundary of the column header. When the pointer changes to a resize arrow, click and drag.
- Right-click the column header > Formatting > Set Width and enter the width in pixels.

Column Visibility

Columns can be hidden while configuring the table. Hidden columns still participate in calculations but are not displayed.

To hide a column:

- Right-click the column name in the Values area of the Component Configuration pane and select Hide. The column name is grayed.
