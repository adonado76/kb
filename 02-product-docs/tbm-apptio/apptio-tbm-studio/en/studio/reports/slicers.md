...
- `!ALL_ROWS` feature is enabled by default in all projects. When
  `!ALL_ROWS` is enabled, slicers will display all values, including those that may not
  have any entries in the current period. If `!ALL_ROWS` is disabled in Project
  Settings, slicers will only display values from the current month, even if annualized tables in the
  report display entries from the entire year.
  ...
  In this example, we have vendor spend from IBM Jan - Mar and vendor spend for Cisco
  only in Jan & Feb.
  - If `ALL_ROWS` is enable, then in Period Mar, both IBM and Cisco would appear in
    the Vendor Name slicer.
  - If `ALL_ROWS` is disabled, then in Period Mar, only IBM would appear in the
    Vendor Name slicer
