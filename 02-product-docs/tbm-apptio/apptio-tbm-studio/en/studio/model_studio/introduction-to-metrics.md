...
- **Table Format** - Specifies the format for the value of the metric when
  displayed in report tables. You can specify formats in HTML or use the [NumberFormat](../formulas-and-functions/functions/numberformat.html "Formats a numeric value into a label (string) using custom patterns for positive and negative numbers, durations, or data size formatting. This function is designed for use in Label-type columns.") function.
