...
*string*: The string from which to extract characters from the right. Note: This parameter
...
*count*: The number of characters to return from the left. If the value is a string, it will
be converted to a number. If conversion fails, the value defaults to 0. Optional (default:
1). An integer specifying the number of characters to be returned. If this argument is not
specified, it defaults to 1. If this argument evaluates to a label or string, the system
will try to convert it to a number. If it cannot, then the value will be zero.
...
Note: White spaces are treated as characters and included in the result.
