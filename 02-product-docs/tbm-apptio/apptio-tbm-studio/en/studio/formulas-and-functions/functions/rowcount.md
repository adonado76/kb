...
Returns a count of the total number of rows in a specified table. If no table is
specified, it returns the number of rows in the current table context.
...
*table*: The name of the table to count rows from. If omitted, counts rows in the current
table context. You may not specify the current table name explicitly. Optional (default:
current table)
