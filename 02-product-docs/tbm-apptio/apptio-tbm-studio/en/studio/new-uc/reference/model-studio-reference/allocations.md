...
Allocations are the core mechanism for distributing value between model objects. They determine
how costs, budgets, and other metrics flow from source objects to target objects based on
configurable rules.
...
**Description:** Distributes value based on the ratio (relative size) of values in a selected
column from the target table. This is the most common allocation type.
...
**Note:** Weight By – Metric and Weight By – Other Driver options are available in TBM Studio
12.5 and later.
...
Warning: Weight by non-numeric: If the weighting column contains any non-numeric value,
weighting is ignored and allocation defaults to even distribution.

Note: Weight by negative: Allocations fail if weighting values include negatives. See “About
weighting and negative numbers” section for workarounds.

See [Create
weighted allocations for step-by-step procedure](../../howtoguides/build-cost-model/create-allocation.html)

**Consumption Allocation**

**Description:** Allocates based on actual units consumed versus total capacity available.
Used when you have specific consumption data showing how much of a service or resource each target
consumes.
...
**Description:** Distributes value equal to the value already in the target table, regardless
of the source amount. This creates a direct pass-through of the target’s existing value.
...
Warning: Standard Value allocations can create over-allocation situations. Monitor
allocation percentages to ensure model integrity.

**Formula Allocation**

**Description:** Uses a custom formula to control allocation distribution. Provides maximum
flexibility for complex allocation scenarios that standard methods cannot address.
...
**Description:** Handles circular allocation patterns where services allocate costs to each
other. Iteratively processes allocations until a precision threshold or maximum iterations is
reached.
...
Warning: Recursive allocations require significantly more processing power. Use
sparingly and with minimal iterations.

*→ See [Configure recursive allocations for detailed setup](../../howtoguides/build-cost-model/config-drivers.html)*

- **[All Allocation Configuration Options](../../../../studio/new-uc/reference/model-studio-reference/allocation-config.html)**
- **[Allocation Order and Precedence](../../../../studio/new-uc/reference/model-studio-reference/allocation-order.html)**
- **[Allocation Calculation Behavior](../../../../studio/new-uc/reference/model-studio-reference/allocation-calc-behaviour.html)**
