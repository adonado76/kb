...
breadcrumb:
  ...
  - "Annual function"
source_path: "SSWRJM/studio/formulas-and-functions/functions/annual.html"
images: []
capability_ids: []
...
*Returns a value for the specified metric adjusted forward or backward by a number of years, based on the selected calendar type.*
...
delta : Optional. The number of years to look forward (positive value) or backward (negative value). If not specified, defaults to 0.

calendarType : Optional. The calendar system to use. Supported values are 'FY' for Fiscal Year (default) and 'CY' for Calendar Year. If not specified, defaults to 'FY'.
...
Annual(Budget, 1, "CY") : Returns the 'Budget' metric for next calendar year.
