...
Um gráfico de colunas exibe os dados usando barras verticais, facilitando a comparação de valores entre categorias. Os gráficos de colunas são ideais quando você deseja enfatizar as diferenças entre os itens ou acompanhar as mudanças ao longo do tempo.

## Quando usar um gráfico de colunas

Use um gráfico de colunas quando desejar:

- Compare valores entre categorias
- Classifique as categorias da mais alta à mais baixa
- Destaque tendências ou diferenças entre categorias

## Adicionar um gráfico de colunas a um relatório

1. Adicione um gráfico de colunas a partir do painel Visualizações na barra de ferramentas
2. Clique no gráfico de colunas para ativar os painéis Dados e Formato.
3. Painel de dados
   ...
   3. Valores – Especifica a(s) métrica(s) exibida(s) como colunas
   ...
   6. Resultados a exibir – Indique o número de colunas a exibir
   7. Configurar classificação – Controla a ordem das colunas por valor em ordem crescente ou decrescente.
4. Painel de formatação
   1. Propriedades gerais – Veja [Propriedades do componente](../components/components.html#abt-comp__comprop)
   2. Propriedades específicas do gráfico de colunas
      1. Categorias
...
![amostra de coluna](../../../../../../03-media/apptio-tbm-studio/resources/images/studio_images/column-vis.png)

Os gráficos de colunas suportam fórmulas personalizadas e dimensões de fórmulas. Para obter mais detalhes, consulte [Fórmulas personalizadas.](../create-first/custom-formula.html "As fórmulas personalizadas (também conhecidas como dimensões de fórmula) permitem definir novas dimensões calculadas utilizando campos existentes no seu modelo de dados. Isso permite uma análise mais profunda e insights mais ricos, sem a necessidade de alterações no conjunto de dados ou no esquema subjacente.")

Os gráficos de colunas também suportam visualizações compatíveis. Para obter mais detalhes, consulte [Visualizações compatíveis](visualizations.html#abt-visual__compvis).

## Gráficos de colunas empilhadas

Os gráficos de colunas podem ser convertidos em colunas empilhadas para exibir as contribuições das subcategorias dentro de cada categoria, sem deixar de mostrar o valor total.
