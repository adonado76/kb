...
| [TimePeriod](functions/timeperiod.html "Retorna o valor de uma métrica especificada na mesma tabela para uma unidade de tempo (por exemplo, mês, trimestre, ano) que é um número especificado de unidades antes ou depois do período atual. Um deslocamento de 0 retorna o valor do período atual. A unidade de tempo é determinada pela visualização selecionada ou por um sufixo de granularidade explícito.") | `TimePeriod(metric,time)` |
