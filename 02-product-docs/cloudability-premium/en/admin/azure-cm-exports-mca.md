...
This guide walks you through the process of connecting your ****Microsoft** **Customer
Agreement** (MCA)** to IBM Cloudability via Azure Cost Management Exports.
...
Note: Billing Account Reader Role can be assigned to an SPN only by a user who has **Billing
Account Owner** e.g Enterprise Administrator /Global Administrator or equivalent.
...
create and grant rights for Cloudability via the CloudabilityCostDataReader role in order to provide
access to the storage account.

1. In Cloudability , navigate to
   1. **Settings** > **Vendor Credentials** > **Add Datasource** > **AZURE** .
   2. Click Next
   3. Select Microsoft Customer Agreement (MCA) - the **Add AZURE Account** panel opens.Or
   4. **Settings** > **Vendor Credentials** > **Ingress** > **AZURE** .
   5. Click Next
   6. Select Microsoft Customer Agreement (MCA)  **- t**he Add a Credential panel opens.
2. Enter the information gathered from your Azure Portal into the corresponding fields
   1. Enter the Azure Billing Account ID
   2. Tenant ID
   3. Subscription ID
...
2. Select the three dots on the right hand side and select Edit next to your **MCA** credential.
