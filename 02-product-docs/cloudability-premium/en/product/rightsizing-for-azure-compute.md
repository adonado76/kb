...
Microsoft Azure Compute. The dashboard shows both the rightsizing and idle (terminate)
recommendations. You can view the recommendations across multiple accounts from a single dashboard.
...
Rightsizing page, select the Azure tab, and then
select the Compute subtab.

![azure compute rightsizing screenshot](../../../../03-media/cloudability-premium/images/azure-compute-1.jpg)
...
If your organization has enabled  Custom Pricing  in Cloudability , the relevant custom
rates will be applied to the cost basis calculations.
...
- On-Demand: The On-Demand cost basis provides a direct comparison between
  ...
  New column based purely on On-Demand Pricing . It does not include any
...
- Effective: The Effective cost basis takes into account the historical
  ...
  In other words, the Effective cost basis shows the
  effective cost of running your current instance. The cost figures for the recommended new instance
  type are based on the on-demand prices. This is because the new configuration may not benefit from
  RIs or SPs. This comparison is the more conservative option. Even if you inadvertently move away
  from RIs or SPs, your new overall rate will still be better. As a result, the overall savings
  reported using this methodology will sometimes be lower. Custom pricing will be applied to these
...
Compute resources for which recommendations have been identified. The table includes the following
columns:
...
![azure recommendation details screenshot](../../../../03-media/cloudability-premium/images/azure-compute-2.jpg)
