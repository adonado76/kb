...
for Amazon Web Services (AWS) Elastic Block Store (EBS). The dashboard shows optimization
...
To view the AWS EBS dashboard, make sure that you have connected Cloudability to the correct AWS
accounts.
...
Access the AWS EBS dashboard

To access the AWS EBS dashboard, open the Cloudability home page, and from the left navigation
menu, select  Optimize > Rightsizing > Advanced. On the Rightsizing page,
select the AWS tab, and then select the EBS subtab.
...
The dashboard contains a rightsizing recommendations table, which provides an overview of EBS
...
- Resource Name : The EBS resource name
...
- Attached VM : EC2 VM to which this EBS is attached to
- Tier : The current EBS storage tier
- Disk Size : Current EBS disk size
- IOPS : Current EBS IOPS
- **Cost** : The current EBS storage cost
- New Tier : The recommended EBS storage tier
- New Disk Size : The recommended EBS storage size
- New IOPS : The recommend EBS IOPS
