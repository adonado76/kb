...
language: "pt-br"
doc_type: "admin"
title: "Conexão com Azure EA - API de detalhes de custos"
breadcrumb:
  - "Cloudability Premium"
  - "Obtendo dados em Cloudability"
  - "Conectar Microsoft Azure"
source_path: "admin/azure-cmapi-ea.html"
images:
  - "images/Azure-EA-Billing-id.png"
  - "images/Azure-EA-Cost-Details-API.png"
...
summary: ""
---
# Conexão com Azure EA - API de detalhes de custos

**Visão geral**

Este guia orienta você passo a passo pelo processo de conexão segura do seu ambiente EA da Azure ao IBM Cloudability por meio das APIs de detalhes de custos do Azure.
...
Observação: O site Cloudability recomenda o uso das exportações do Azure para o credenciamento no Microsoft Azure. Clique [aqui](azure-cm-ea.html) para configurar as exportações d Azure. Microsoft Azure também recomenda o uso de [exportações](https://learn.microsoft.com/en-us/azure/cost-management-billing/automate/usage-details-best-practices "(Abre em uma nova guia ou janela)") como uma prática recomendada para grandes conjuntos de dados. (Mais de 2 GB por mês)
...
O processo de credenciamento para a API de Detalhes de Custos do “ Azure ” da Cloudability requer duas etapas principais:
...
1. **No portal “ Azure ”:**
   ...
   1. Localize **o ID da** sua conta de cobrança (ID de inscrição).
      1. Pesquise “**Gerenciamento** de Custos + Faturamento” e selecione-o na lista para abrir a página “Visão geral do Gerenciamento de Custos + Faturamento”.
      ...
      3. **Acesse “Configurações”, clique em “Propriedades”,** copie e salve **o ID** da sua conta de cobrança (ID de cadastro).
         ...
         ![](../../../../03-media/cloudability-premium/images/Azure-EA-Billing-id.png)
   ...
   3. Obtenha seu **ID de assinatura**
      1. Da lista de assinaturas vinculadas à mesma conta de cobrança.
...
2. **Em Cloudability - Configurar credenciais para a conta de cobranç Azure**
   ...
   5. Um script do PowerShell será baixado.
      ...
      ![](../../../../03-media/cloudability-premium/images/Azure-EA-Cost-Details-API.png)
3. **No Portal Azure - Carregue o script PowerShell**
   ...
   4. Além disso, atribuirá uma função chamada “**Leitor de** matrículas” à conta de cobrança utilizada para acessar a API de Detalhes de Custos por meio de Cloudability.
...
Caso a verificação falhe, tente novamente após 15 minutos.
...
Como próximo passo, siga as instruções sobre **“Configuração de credenciais avançadas”, “ Azure ” e “Dimensionamento adequado e planejamento de instâncias reservadas** ”. A configuração dessas permissões ajudará a importar as tags de assinatura do Azure e os dados de utilização, o que facilita o uso dos recursos do Optimize no Cloudability.
...
3. **Como faço para atualizar para o modelo mais recente do PowerShell para permissões?**
   ...
   1. Para regenerar o modelo PowerShell e reconfigurar seu acesso ao Azure, faça o seguinte:
      ...
      1. No site Cloudability, acesse **Configurações > Credenciais do fornecedor > Azure**.
