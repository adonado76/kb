...
language: "pt-br"
doc_type: "admin"
title: "Conexão com Azure EA - Gerenciamento de custos de exportação"
breadcrumb:
...
summary: ""
---
# Conexão com Azure EA - Gerenciamento de custos de exportação

**Visão geral**

Este guia orienta você passo a passo pelo processo de conexão **do** seu Contrato Empresarial da Microsoft (EA) ao IBM Cloudability por meio das exportações de gerenciamento de custos do Azure.
...
- Redator de Matrículas ou função equivalente para atribuir a função de Leitor de Matrículas
...
- Utiliza a função “Leitor de Inscrições”, que fornece acesso somente para leitura aos dados de faturamento da empresa

Observação: Uma função de “ EnrollmentReader ” só pode ser atribuída a um SPN por um usuário que possua a função de “enrollment writer”, por exemplo, Administrador Corporativo/Administrador Global ou equivalente.

**Etapa 1 – Portal Azure – Criar uma exportação dos detalhes de custos do Azure**

Observação: O site Cloudability recomenda o uso das exportações do Azure para a credenciação no Microsoft Azure.
...
6. Certifique-se de que as configurações abaixo estejam aplicadas à conta de armazenamento configurada para exportações:
   ...
   4. Clique em Gerenciar
      ...
      2. Na seção “Escopo de acesso à rede pública”, selecione uma das opções abaixo
...
Depois de reunir as informações listadas na tabela, você pode inseri-las no site Cloudability para atualizar sua credencial de EA.

Cloudability usará essas informações para gerar um script PowerShell. Quando o script for executado a partir do seu Cloud Shell no Portal Azure ou a partir do PowerShell local, restrito ao seu locatário, ele criará uma Entidade de Serviço e concederá direitos para Cloudability por meio da função CloudabilityCostDataReader, a fim de fornecer acesso à conta de armazenamento.

1. No site Cloudability, acesse
   1. **Configurações** > **Credenciais do fornecedor** > **Adicionar fonte de dados** > **AZURE**.
   2. Clique em “Próximo”
   3. Selecione “Contrato Empresarial (EA)” — o painel **“Adicionar conta do Azure”** é exibido.Ou
   4. **Configurações** > **Credenciais do fornecedor** > **Ingress** > **AZURE**.
   5. Clique em “Próximo”
   6. Selecione “Acordo Empresarial (EA) **” —** o painel “Adicionar credencial” é exibido.
2. Insira as informações coletadas no seu Portal Azure nos campos correspondentes
   1. Insira o ID de inscrição do Azure (ID da conta de cobrança)
   2. ID do locatário
   3. ID da assinatura
...
A etapa final é verificar sua credencial atualizada do EA do Cloudability.

1. Volte à página de credenciais do Cloudability Azure em **Configurações > Credenciais de fornecedor > AZURE**.
2. Selecione os três pontos à direita e escolha “Editar” ao lado das suas credenciais **de EA**.
...
Clique aqui para [configurar o planejamento de ajuste de capacidade e instâncias reservadas d Azure,](azure-advanced-rightsizing.html) a fim de credenciar as assinaturas do Azure
...
6. **Você apoia a exportação de custos em perfis de** **faturamento/departamentos?**Sim, é possível – Entre em contato com a equipe de suporte da IBM Cloudability para obter mais informações.
