...
language: "pt-br"
doc_type: "product"
title: "Azure Inventário de recursos"
breadcrumb:
...
summary: ""
---
# Azure Inventário de recursos

Azure O recurso Resource Inventory (Inventário de recursos) do site Cloudability permite que você produza uma lista confiável dos recursos de nuvem do site Azure que existiam e foram faturados durante um período de relatório específico. Você tem a flexibilidade de criar essa lista a partir de recursos que abrangem várias contas e escolher entre diferentes medidas (dimensões e métricas) para exibir os detalhes relevantes para elas. Essas informações, disponíveis para cada recurso de nuvem, unificam o faturamento, a utilização e os metadados descritivos para oferecer uma visão mais abrangente do inventário.

## Introdução

Azure O recurso de relatório Resource Inventory deve estar ativado em sua Cloudability Org. Você deve entrar em contato com o seu TAM/CSM/ponto de contato da conta em Apptio para fazer isso. Depois de ativado, aguarde de 2 a 3 dias úteis para que os dados comecem a aparecer em seus relatórios de inventário.
...
- Para cada serviço do site Azure, há suporte para um conjunto específico de medidas.
...
Para cada serviço (por exemplo: computação ou discos gerenciados), os KPIs serão exibidos na parte superior do relatório para fornecer a você informações resumidas importantes, como instâncias de computação inativas e instâncias de computação não marcadas. Você pode clicar e filtrar o relatório de inventário com base nesses KPIs para detalhar os dados específicos do relatório que está procurando.

Janelas de data de relatório de inventário

Você pode selecionar o período no seletor de datas para o qual deseja visualizar os dados de inventário de cada serviço. O intervalo máximo permitido para seleção é de 31 dias. Ao ativar esse recurso, os dados do inventário de recursos serão preenchidos novamente até o início do mês atual. Com o passar do tempo, os dados de inventário estarão disponíveis em um período contínuo de três meses. A qualquer momento, você poderá acessar os dados de inventário de recursos para o mês atual e para os dois meses anteriores.
...
Clicar em Exportar no canto superior direito da tabela do relatório exportará todas as colunas de inventário selecionadas com qualquer filtro aplicado. No entanto, para exportar os dados completos do inventário para o mês e o serviço selecionados com todas as colunas compatíveis, clique no botão Exportar no canto superior direito do menu suspenso de filtros na barra de ferramentas da guia.
...
1. Para acessar Azure Resource Inventory, navegue até Insights> Resource Inventory e clique na guia Azure guia.
...
| --- | --- |
| Computação do Azure | |
| Total de instâncias de computação | Número total de instâncias de computação em seu inventário para o período selecionado. |
| Instâncias de computação inativas | Número de instâncias de computação que estão em um destes estados: Parando, Parado, Desalocado ou Desalocando. |
| Custo (total) | Custo (total) para as instâncias de computação. |
| Instâncias de computação não marcadas | Número de instâncias de computação que não têm uma tag. |
| Disco gerenciado | |
| Total de discos | Número total de discos gerenciados em seu inventário para o período selecionado. |
| Discos inativos | Número de discos gerenciados que estão no estado "Disponível". |
| Custo (total) | Custo (total) para os discos gerenciados. |
| Discos sem tags | Número de discos gerenciados que não têm uma tag. |
| Discos não conectados | Número de discos gerenciados que não estão conectados a nenhum VM. |
| Banco de dados | |
| Total de bancos de dados | Número total de bancos de dados em seu inventário para o período selecionado. |
| Bancos de dados sem tags | Número de bancos de dados que não têm uma tag. |
| Custo (total) | Custo (total) para os bancos de dados. |
